      ...
      >
        ...
        <el-col :span="17" class="itemContent">
          <div class="title">
            ...
            <el-button type="danger" @click="deleteItem(item.id)" class="delete" v-if="isUser">删除</el-button>
          </div>
          <div class="info">
            <span class="time">
...
export default {
  ...
  computed: {
    ...
    isUser () {
      return this.targetUserData.userId === this.userId
    }
  },
  methods: {
    checkOrder (index) {
    ...
    deleteItem (id) {
      this.$confirm('此操作将永久删除该帖子，是否继续？', '提示', {
        ...
        axios.post('/user/deleteItem', {id, userId: this.targetUserData.userId}).then(res => {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
  ...
  created () {
    axios.post('api/getPostData', {arr: this.targetUserData.posts, userId: this.targetUserData.userId}).then(res => {
      this.postData = res.data.reverse()
    }).catch(err => {
      console.log(err)
    })
  }
